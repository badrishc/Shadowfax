...
class AsyncPendingReadContext : public PendingContext<K> {
 ...
 public:
  ...
  virtual void GetAtomic(void* rec) = 0;
};
...
class PendingReadContext : public AsyncPendingReadContext<typename RC::key_t> {
 ...
 public:
  ...
  inline void GetAtomic(void* rec) final {
...
/// FASTER's internal CondUpsert() context(s).

/// A Conditional upsert that has gone asynchronous and lost it's type
/// information. Supports methods that a read and upsert context would.
template <class K>
class AsyncPendingCondUpsertContext : public PendingContext<K> {
 public:
  typedef K key_t;
 protected:
  AsyncPendingCondUpsertContext(IAsyncContext& caller_context_,
                                AsyncCallback caller_callback_)
    : PendingContext<key_t>(OperationType::CondUpsert,
                            caller_context_,
                            caller_callback_)
  {}

  /// The deep copy constructor.
  AsyncPendingCondUpsertContext(AsyncPendingCondUpsertContext& other,
                                IAsyncContext* caller_context)
    : PendingContext<key_t>(other, caller_context)
  {}
...
/// A synchronous conditional upsert with it's type information preserved.
template <class UC>
class PendingCondUpsertContext : public AsyncPendingCondUpsertContext<typename
                                                                      UC::key_t>
{
 public:
  typedef typename UC::key_t key_t;
  typedef typename UC::value_t value_t;
  typedef Record<key_t, value_t> record_t;

  PendingCondUpsertContext(UC& caller_context_, AsyncCallback caller_callback_)
    : AsyncPendingCondUpsertContext<key_t>(caller_context_, caller_callback_)
  {}

  /// The deep copy constructor.
  PendingCondUpsertContext(PendingCondUpsertContext& other,
                           IAsyncContext* caller_context_)
    : AsyncPendingCondUpsertContext<key_t>(other, caller_context_)
  {}

 protected:
  Status DeepCopy_Internal(IAsyncContext*& context_copy) final {
    return IAsyncContext::DeepCopy_Internal(*this,
                                          PendingContext<key_t>::caller_context,
                                          context_copy);
  }

 private:
  inline const UC& cond_upsert_context() const {
    return *static_cast<const UC*>(PendingContext<key_t>::caller_context);
  }

  inline UC& cond_upsert_context() {
    return *static_cast<UC*>(PendingContext<key_t>::caller_context);
  }

 public:
  inline const key_t& key() const final {
    return cond_upsert_context().key();
  }

  inline void Get(const void* rec) final {
    const record_t* record = reinterpret_cast<const record_t*>(rec);
    cond_upsert_context().Get(record->value());
  }

  inline void GetAtomic(void* rec) final {
    record_t* record = reinterpret_cast<record_t*>(rec);
    cond_upsert_context().GetAtomic(record->value());
  }

  inline void Put(void* rec) final {
    record_t* record = reinterpret_cast<record_t*>(rec);
    cond_upsert_context().Put(record->value());
  }

  inline bool PutAtomic(void* rec) final {
    record_t* record = reinterpret_cast<record_t*>(rec);
    return cond_upsert_context().PutAtomic(record->value());
  }

  inline constexpr uint32_t value_size() const final {
    return cond_upsert_context().value_size();
  }
};
