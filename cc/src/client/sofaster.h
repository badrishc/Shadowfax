...
/// A flag indicating whether a session to the target has been preallocated.
thread_local bool prealloc = false;
...
/// Migration percentage. Required for view changes.
thread_local uint32_t pct = 10;
...
class Sofaster {
 public:
  ...
  typedef CondUpsertRequest<K, V> cond_upsert_t;
  ...
  /// Forward declaration for updateHashRange.
  struct HashRange;

 private:
  /// Updates the hash range assigned to a session on the sessions map.
  inline void updateHashRange(struct HashRange orgR, struct HashRange newR) {
    size_t idx = sessionMap.at(orgR);
    sessionMap.erase(orgR);

    sessionMap.emplace(newR, idx);
  }
 ...
 public:
  ...
  /// Enqueues a conditional upsert request onto a passed in session.
  ///
  /// \param ctxt
  ///    Context for the upsert. Will be passed in as an argument to the
  ///    callback once a response to the upsert is received.
  ...
  ///    Callback function to be invoked once a response to the request is
  ///    received from SoFASTER.
  inline void condUpsert(UpsertContext& ctxt, AsyncCallback cb)
  {
    // Enqueue a context and callback. The callback will be invoked with the
    // context as an argument when a response to this request is received.
    session_t& session = getSession(ctxt.key());
    ...
    // Buffer the request on the session and try to transmit.
    auto req = cond_upsert_t(ctxt.key(), ctxt.val());
    bool retry = true;
    while (retry) {
      retry = !session.buffer.bufferRequest(reinterpret_cast<uint8_t*>(&req),
                                            sizeof(cond_upsert_t));
      if (!flushSession(session)) handleViewChange(ctxt.key());
    }
  }
 ...
 private:
  ...
  inline void handleViewChange(key_t& key) {
    ...
    // First, update the hash range on the original session to the source,
    // and add a new session for the hash range migrated to the target.
    uint64_t split = ~0UL / (sessions.size() - 1);
    updateHashRange(HashRange(hash_t(0UL), hash_t(split)),
                    HashRange(hash_t((pct * (split / 100)) + 1),
                              hash_t(split)));
    addHashRange(0UL, pct * (split / 100), std::string(""),
                 std::to_string(22790 + id), sessions.size() - 1);

    // Next, shuffle requests sent out on this session after the view change
    // on the server between itself and the session that now owns the migrated
    // hash range. Since adding a hash range could invalidate any existing
    // references, we obtain a new reference to the session here.
    session_t& session = sessions[sIdx];
    shuffle(session);

    // Finally, set the source session to open again. Buffered requests will
  ...
  /// Takes care of shuffling requests on the session from the appropriate
  /// offset during a view change on the session.
  inline void shuffle(session_t& session) {
    // The idea is simple; we first reset our buffer to the last request
    // that was successfully acked by the server.
    auto tail = session.buffer.getTailAddr();
    session.buffer.resetTx();

    // Next, starting from this request to the most recent, we just reissue
    // requests into session buffers based on the new ownership mappings.
    // Once this is done, the transmit pointer within the buffer would have
    // clocked over to the current tail and we can return to normal.
    while (session.buffer.getTxAddr() < tail) {
      reissue(session);
    }
  }

  /// The actual shuffle during a view change happens in here.
  inline void reissue(session_t& session) {
    // Retrieve requests to be reissued.
    auto tx = session.buffer.getTx();
    auto req = std::get<0>(tx);
    auto size = std::get<1>(tx);
    auto left = size;

    // Clock over them so that they are never transmitted afterwards from
    // this session, also making space in the buffer while doing so.
    session.buffer.advanceTx(size);
    session.buffer.advanceHead(size);

    // Re-enqueue them based on the new ownership mappings.
    while (left > 0) {
      switch(*req) {
      case static_cast<int>(Opcode::READ): {
        auto reaD = reinterpret_cast<read_t*>(req);
        auto ctxt = session.contexts.front();

        session_t& dest = getSession(reaD->key);
        dest.buffer.bufferRequest(req, sizeof(read_t));
        dest.contexts.emplace_back(ctxt);

        session.contexts.pop_front();
      ...
      case static_cast<int>(Opcode::UPSERT): {
        auto upst = reinterpret_cast<upsert_t*>(req);
        auto ctxt = session.contexts.front();

        session_t& dest = getSession(upst->key);
        dest.buffer.bufferRequest(req, sizeof(upsert_t));
        dest.contexts.emplace_back(ctxt);

        session.contexts.pop_front();

        req += sizeof(upsert_t);
        left -= sizeof(upsert_t);
        break;
      }

      case static_cast<int>(Opcode::RMW): {
        auto rmw = reinterpret_cast<rmw_t*>(req);
        auto ctx = session.contexts.front();

        session_t& dest = getSession(rmw->key);
        dest.buffer.bufferRequest(req, sizeof(rmw_t));
        dest.contexts.emplace_back(ctx);
      ...
      default:
        logMessage(Lvl::ERROR,
          "Encountered unknown request during view change");
        break;
      }
    }
  }
 ...
 public:
  ...
  /// Initiates a migration of a hash range [\param lHash, \param rHash) away
  /// from a SoFASTER server (\param sIP). Returns true if the migration was
  /// successfully initiated. False, otherwise.
  ///
  /// \param sAdmn
  ///    The port that the source listens to for control RPCs.
  /// \param dIP
  ///    Destination IP address.
  /// \param dPort
  ///    Base port on which destination worker threads listen for connections.
  inline bool migrate(uint64_t lHash, uint64_t rHash, const std::string& sIP,
                      uint16_t sAdmn, const std::string& dIP, uint16_t dPort)
  {
    connection_t* conn = connHandle->connect(sIP, std::to_string(sAdmn));

    // Context passed down to the sessions layer when sending out the RPC.
    // Contains the hash range to be migrated, the destination's IP address
    // and the base port on which the destination's workers are listening.
    struct Context {
      HashRange range;
      ...
      Context(HashRange r, const std::string& ip, uint16_t port)
       : range(r)
    ...
    // Send out the Migrate() request to the appropriate server. Since this is
    // a control message, we don't use sessions here; just raw connections.
    Context context(HashRange(hash_t(lHash), hash_t(rHash)), dIP, dPort);
    auto sendCb = [](void* ctxt, uint8_t* request) {
      auto ctx = reinterpret_cast<Context*>(ctxt);

      // Add in a generic RPC header so that this works with TCP too.
      auto hdr = reinterpret_cast<RequestBatchHdr*>(request);
      hdr->size = sizeof(MigrateRequest);
      request += sizeof(RequestBatchHdr);

      auto req = reinterpret_cast<MigrateRequest*>(request);
      req->op = Opcode::MIGRATE;
      req->firstHash = ctx->range.lHash;
      req->finalHash = ctx->range.rHash;
      req->destIP = ctx->destIP;
      req->destPort = ctx->desPort;

      return (uint32_t) (sizeof(MigrateRequest) + sizeof(RequestBatchHdr));
    };
    conn->sendRpc(reinterpret_cast<void*>(&context), sendCb);

    // We timeout and return false if the server does not respond within 10
    // seconds of sending out the request.
    auto start = high_resolution_clock::now();
    auto stop = start + seconds(10);

    // Poll for a response until we either receive one or timeout.
    bool done = false;
    auto recvCb = [](void* ctxt, uint8_t* request, uint8_t* response) {
      auto doneFlag = reinterpret_cast<bool*>(ctxt);
      ...
      if (res->status == Status::SUCCESS) *doneFlag = true;

      return (uint32_t) 0;
    };

    while (!done  && high_resolution_clock::now() < stop) {
      respPoller->poll();
      conn->receive(reinterpret_cast<void*>(&done), recvCb);
    }

    return done;
  }

  /// Sends out a Compact() RPC request to a SoFASTER server with a given IP
  /// address (`ip`). `untilAddress` is the log offset upto which the
  /// server must perform compaction.
  inline bool compact(uint64_t untilAddress, std::string& ip) {
    connection_t* conn = connHandle->connect(ip, std::to_string(50000));

    // Send out the compaction request over the connection itself. Don't open
    // a session because this is on the control path.
    auto sendCb = [](void* ctxt, uint8_t* request) {
      uint64_t* until = reinterpret_cast<uint64_t*>(ctxt);

      // Add in a generic RPC header so that this works with TCP.
      auto hdr = reinterpret_cast<RequestBatchHdr*>(request);
      hdr->size = sizeof(CompactRequest);
      request += sizeof(RequestBatchHdr);

      auto req = reinterpret_cast<CompactRequest*>(request);
      req->op = Opcode::COMPACT;
      req->until = *until;

      return (uint32_t) (sizeof(CompactRequest) + sizeof(RequestBatchHdr));
    };
    conn->sendRpc(reinterpret_cast<void*>(&untilAddress), sendCb);

    // Wait until we receive a response to the RPC request.
    bool done = false;
    auto recvCb = [](void* ctxt, uint8_t* request, uint8_t* response) {
      auto doneFlag = reinterpret_cast<bool*>(ctxt);
 ...
 private:
                                ...
                                uint8_t* response)
  {
    ...
    if ((hdr->status == Status::RETRY_LATER ||
         hdr->status == Status::RETRY_STRICT) &&
    ...
    // The server did not execute this batch because it has too many waiting
    // requests. Simply return and reissue the batch after the backoff interval.
    if (hdr->status == Status::RETRY_STRICT) return 0;
    ...
    while (bytes) {
      ...
      case static_cast<int>(Opcode::CONDITIONAL_UPSERT): {
        auto res = reinterpret_cast<CondUpsertResponse*>(response);
        handleCondUpsert(session, res);

        bytes -= sizeof(CondUpsertResponse);
        response += sizeof(CondUpsertResponse);
        break;
      }
  ...
  /// Handles a response to a conditional upsert request that was issued to
  /// SoFASTER.
  ///
  /// \param session
  ///    Pointer to the session the response was received on.
  /// \param response
  ///    Pointer to the conditional upsert response.
  inline static void handleCondUpsert(session_t* session,
                                      CondUpsertResponse* response)
  {
    switch (response->status) {
    // The conditional upsert request executed successfully at the server.
    // Retrieve the requests context and callback from the session. Invoke
    // the callback with the context passed in as an argument.
    case Status::SUCCESS:
      session->execEarliestCb(FASTER::core::Status::Ok);
      session->nResponses++;
      break;
    ...
    default:
      logMessage(Lvl::ERROR, "CondUpsert failed, session %s, unexpected status",
                 session->sessionUID.ToString().c_str());
      assert(false);
      break;
    }
