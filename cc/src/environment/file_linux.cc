                  ...
                  bool* exists, uint64_t bytes)
{
  ...
  if(fd_ == -1) {
    logMessage(Lvl::ERROR, "Failed to open file %s (errno %d)",
               filename_.c_str(), errno);
    return Status::IOError;
  }

  if (bytes > 0) {
    int r = ::ftruncate(fd_, bytes);
    if (r < 0) {
      logMessage(Lvl::ERROR,
                 "Failed to set size of file %s to %lu Bytes (errno %d)",
                 filename_.c_str(), bytes, errno);
      return Status::IOError;
    }
  }
...
bool QueueIoHandler::TryComplete() {
  ...
  int result =
    ::io_getevents(io_object_[Thread::id() - 1], 1, 128, events, &timeout);
  if(result >= 1) {
    for (auto i = 0; i < result; i++) {
      io_callback_t callback = reinterpret_cast<io_callback_t>(events[i].data);
      callback(io_object_[Thread::id() - 1], events[i].obj, events[i].res,
               events[i].res2);
    }
    return true;
  } else {
...
Status QueueFile::Open(FileCreateDisposition create_disposition, const FileOptions& options,
                       QueueIoHandler* handler, bool* exists, uint64_t size) {
  int flags = 0;
  if(options.unbuffered) {
    flags |= O_DIRECT;
  }
  RETURN_NOT_OK(File::Open(flags, create_disposition, exists, size));
  if(exists && !*exists) {
    return Status::Ok;
  }
...
Status QueueFile::ScheduleOperation(FileOperationType operationType, uint8_t* buffer,
  ...
  int result = ::io_submit(io_object_[Thread::id() - 1], 1, iocbs);
  if(result != 1) {
    return Status::IOError;
  }
