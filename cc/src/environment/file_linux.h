...
class File {
 ...
 protected:
  Status Open(int flags, FileCreateDisposition create_disposition,
              bool* exists = nullptr, uint64_t bytes=0);
...
class QueueIoHandler {
 ...
 public:
  ...
  QueueIoHandler(size_t max_threads)
    : io_object_{ } {
    for (int i = 0; i < Thread::kMaxNumThreads; i++) {
      int result = ::io_setup(kMaxEvents, &io_object_[i]);
      assert(result >= 0);

      if (result < 0) {
        logMessage(Lvl::ERROR,
                   "Failed to setup AIO object for thread %d (result %d)",
                   i, result);
      }
    }
  }
  ...
  QueueIoHandler(QueueIoHandler&& other) {
    for (int i = 0; i < Thread::kMaxNumThreads; i++) {
      io_object_[i] = other.io_object_[i];
      other.io_object_[i] = 0;
    }
  }
 ...
 private:
  ...
  io_context_t io_object_[Thread::kMaxNumThreads];
};
...
class QueueFile : public File {
 public:
  ...
  Status Open(FileCreateDisposition create_disposition, const FileOptions& options,
              QueueIoHandler* handler, bool* exists = nullptr, uint64_t size=0);
